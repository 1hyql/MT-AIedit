 ...
 *
 * 说明：MT 插件 UI 对象允许在后台线程更新（官方 Demo 的 LoadingDialog 等均在
 *       worker 线程回调），因此流式回调里直接更新控件是安全的。
 */
public final class AiEditDialog {
    ...
    public static void show(@NonNull PluginUI pluginUI, @NonNull TextEditor editor) {
        ...
        final String fullText = editor.subText(0, editor.length());
        ...
        final int maxTokens = Prefs.getInt(prefs, Prefs.KEY_MAX_TOKENS, 4096);
                    ...
                    public void onFinished(@NonNull String ignored) {
                        ThreadUtil.runOnUiThread(() -> {
                            if (cancelled.get()) {
                                ...
                                status.setText(context.getString("dialog_status_done", resultText.length()));
                            }
